        ...
        <ul className="list-none">
          {notifications.map((notification) => (
            <li key={notification.id} className="mb-4">
              ...
              <p className="text-navy-400 mb-1">{notification.description}</p>
              <p className="text-navy-800">{notification.date}</p>
            </li>
          ))}
        </ul>
      ) : (
        <p className="text-navy-800">No notifications to display.</p>
      )}
    </div>
  );
};
